...
export default function EventDetails() {
  ...
  async function load() {
    ...
    if (user) {
      ...
      const isRegistered = res.data.registrations.some(
  ...
  //     setRegistered(false);
  //     showToast('info', 'Unregistered successfully');
  //   } catch (err) {
  //     console.log(err);
  //   }
  // }
  ...
  async function unregister() {
    try {
      await axios.delete(`/api/registrations/${id}/unregister`);
      setRegistered(false);
      showToast('info', 'Unregistered successfully');
    } catch (err) {
      console.log(err);
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-3">
            ...
            {/* {user?.role === "customer" && (<button className="btn" onClick={register}> Register </button> )} */}
            {/* {user?.role === "customer" && (
  <button
    className="btn"
    onClick={register}
  ...
  >
    {registered ? "Registered" : "Register"}
            ...
            {/* {user?.role === "customer" && (
              registered ? (
                <button className="btn-outline" onClick={unregister}>
                  Unregister
                </button>
              ) : (
